    ...
    class LimoWrapper : public rclcpp::Node
    {
        ...
        public:
            ...
            LimoWrapper() : Node("fast_limo", 
                                ...
                                .automatically_declare_parameters_from_overrides(true) ) 
                {
                    ...
                    // Set up subscribers (support both PointCloud2 and Livox CustomMsg)
                    // For LIVOX: check if using CustomMsg format, otherwise use PointCloud2
                    bool use_livox_custom_msg = false;
                    if (config.sensor_type == static_cast<int>(fast_limo::SensorType::LIVOX)) {
                        try {
                            rclcpp::Parameter use_custom_p = this->get_parameter("use_livox_custom_msg");
                            use_livox_custom_msg = use_custom_p.as_bool();
                        } catch (const rclcpp::ParameterTypeException&) {
                            // Parameter not set, default to false (use PointCloud2)
                            use_livox_custom_msg = false;
                        }
                        
                        if (use_livox_custom_msg) {
                            RCLCPP_INFO(this->get_logger(), "Subscribing to Livox LiDAR (CustomMsg format)" ANSI_COLOR_GREEN ANSI_COLOR_RESET);
                            livox_sub_ = this->create_subscription<livox_ros_driver2::msg::CustomMsg>(
                                            config.topics.lidar, 1, std::bind(&LimoWrapper::livox_callback, this, std::placeholders::_1), lidar_opt);
                        } else {
                            RCLCPP_INFO(this->get_logger(), "Subscribing to Livox LiDAR (PointCloud2 format)" ANSI_COLOR_GREEN ANSI_COLOR_RESET);
            ...
            void livox_callback(const livox_ros_driver2::msg::CustomMsg & msg) {
                ...
                // Convert CustomMsg to PCL point cloud
                pcl::PointCloud<PointType>::Ptr pc_ (std::make_shared<pcl::PointCloud<PointType>>());
                this->fromLivoxToPCL(msg, pc_);

                // Use message header timestamp as the reference time
                // The individual point timestamps are already set in fromLivoxToPCL
                double timestamp = rclcpp::Time(msg.header.stamp).seconds();

                loc.updatePointCloud(pc_, timestamp);
                
                // Continue with publishing (same as lidar_callback)
                this->publishPointClouds(loc);
            }
            ...
            void publishPointClouds(fast_limo::Localizer& loc) {
        ...
        /* ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////// 
           ...
           ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////// */
            ...
            void fromLivoxToPCL(const livox_ros_driver2::msg::CustomMsg& in, pcl::PointCloud<PointType>::Ptr& out){
                out->clear();
                out->reserve(in.point_num);
                
                // timebase is in nanoseconds
                double timebase_sec = static_cast<double>(in.timebase) / 1e9;
                
                for (const auto& livox_point : in.points) {
                    PointType pcl_point;
                    ...
                    // Calculate absolute timestamp: timebase (seconds) + offset_time (nanoseconds) / 1e9
                    // For Livox, timestamp is absolute time in seconds * 10e9 (nanoseconds)
                    // offset_time is relative to timebase in nanoseconds
                    double point_timestamp = timebase_sec + (static_cast<double>(livox_point.offset_time) / 1e9);
                    pcl_point.timestamp = point_timestamp;
